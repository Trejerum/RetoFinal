...
public class MantGeneros extends JFrame implements ActionListener {
	...
	/**
	 * Esta Funcion comprueba primero si el textfield esta lleno, despues comprueba
	 * si lo introducido en el textfield no esta en la tabla de generos y si todo
		...
		try {
			if (tfAñadir.getText().length() == 0) {
				...
				if (genero.getNomGenero().length() < 4) {
					genero.setCodGenero(
							genero.getNomGenero().substring(0, genero.getNomGenero().length()).toUpperCase());
				} else {
